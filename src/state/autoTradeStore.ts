 ...
 * 自動売買のスイッチ。
 *
 * **現在この操作盤は画面に出していない。** 総当たり検証(1年・2160通り)で、
 * 実際にTP/SLで決済した場合に優位性のある組み合わせが1つも確認できなかったため、
 * 撃つ根拠が無い。根拠の無い自動売買は損失を自動化するだけなので、
 * ホームからパネルを外してある(src/screens/HomeScreen.tsx)。
 *
 * 判定・注文の組み立て・上限・停止条件は動く状態で残してあるので、
 * 優位性のある判定が見つかった時点でパネルを戻せば再開できる。
 *
